...
url = 'https://raw.githubusercontent.com/biftor/telegram-configs-collector/main/telegram channels.json'
...
for url_link in subscription_links:
    try:
        ...
        if 'biftor' not in url_link:
            raw_array_links_content.append((url_link, links_content))
        elif 'biftor' in url_link and 'channels' in url_link:
...
# Extract IPs from configurations
print("\n" + "="*50)
print("EXTRACTING IP ADDRESSES FROM CONFIGURATIONS")
print("="*50)

# Create ips directory if it doesn't exist
if not os.path.exists('./ips'):
    os.mkdir('./ips')

# Create a temporary config file with all configurations for IP extraction
temp_config_file = './temp_all_configs.txt'
all_configs_for_ip_extraction = array_mixed.copy()

# Remove header/footer configs that are not actual proxy configs
configs_to_extract = []
for config in all_configs_for_ip_extraction:
    if (config.startswith('vmess://') or config.startswith('vless://') or 
        config.startswith('trojan://') or config.startswith('ss://') or 
        config.startswith('hysteria://') or config.startswith('hy2://') or
        config.startswith('tuic://') or config.startswith('juicity://')):
        configs_to_extract.append(config)

# Write configs to temporary file
with open(temp_config_file, 'w', encoding='utf-8') as f:
    for config in configs_to_extract:
        f.write(f"{config}\n")

# Import and run the IP extraction
try:
    import sys
    sys.path.append('.')
    from extract_ips import extract_ips_from_config
    
    # Extract IPs and save to files with IPv4/IPv6 separation
    extract_ips_from_config(
        temp_config_file, 
        './ips/all_ipv4.txt', 
        './ips/cloudflare_ipv4.txt',
        './ips/all_ipv6.txt',
        './ips/cloudflare_ipv6.txt'
    )
    
    print(f"IP extraction completed successfully!")
    print(f"Files created:")
    print(f"- All IPv4 IPs: ./ips/all_ipv4.txt")
    print(f"- All IPv6 IPs: ./ips/all_ipv6.txt")
    print(f"- Cloudflare IPv4 IPs: ./ips/cloudflare_ipv4.txt")
    print(f"- Cloudflare IPv6 IPs: ./ips/cloudflare_ipv6.txt")
    
except Exception as e:
    print(f"Error during IP extraction: {e}")
...
with open('./readme.md', 'w') as file:
    file.write(readme + '\n' + create_country_table('./countries') + ip_section + '\n' + stats)
